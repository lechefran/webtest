...
func TestAutoGeneratedLogFileIsReusedAcrossRequests(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("Hello, World!"))
	}))
	defer server.Close()
	...
	client := InitWebClient().Options(WebClientOptions{WriteToFile: true})
	...
	if len(logFiles) != 1 {
		t.Fatalf("expected exactly 1 autogenerated log file, got %d (%v)", len(logFiles), logFiles)
	}

	content, err := os.ReadFile(tmpDir + "/" + logFiles[0])
	if err != nil {
		t.Fatal(err)
	}
	lines := strings.Split(strings.TrimSpace(string(content)), "\n")
	if len(lines) != 2 {
		t.Fatalf("expected 2 call log lines in reused file, got %d", len(lines))
	}
}

func TestClearGeneratedLogFilePathCreatesNewAutoGeneratedFile(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte("Hello, World!"))
	}))
	defer server.Close()
	...
	client.ClearGeneratedLogFilePath()
	time.Sleep(1100 * time.Millisecond)
	...
	if len(logFiles) != 2 {
		t.Fatalf("expected 2 autogenerated log files after clearing path, got %d (%v)", len(logFiles), logFiles)
	}
}
